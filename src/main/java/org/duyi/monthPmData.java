...
public class monthPmData {
	...
	public static void main(String[] args) throws UnknownHostException {
		...
		DBCollection users_month = db71.getCollection("pmdata_month");
		...
		for (int i = 1; i < 13; i++) {
			if (i < 10) {
				pattern = Pattern.compile("^2014-0" + i + ".*$",
						Pattern.CASE_INSENSITIVE);
			} else {
				pattern = Pattern.compile("^2014-" + i + ".*$",
						Pattern.CASE_INSENSITIVE);
			}
