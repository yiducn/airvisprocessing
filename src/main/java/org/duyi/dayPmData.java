...
public class dayPmData {
	...
	public static void main(String[] args) throws UnknownHostException {
		...
		DBCollection users_day = db71.getCollection("pmdata_day");
		BasicDBObject cond = new BasicDBObject();
		// 正则匹配
		Pattern pattern = null;
		for (int j = 1; j < 31; j++) {
			if (j < 10) {
				pattern = Pattern.compile("^2014-11-0" + j + ".*$",
						Pattern.CASE_INSENSITIVE);
			} else {
				pattern = Pattern.compile("^2014-11-" + j + ".*$",
						Pattern.CASE_INSENSITIVE);
			}
			cond.put("time_point", pattern);
				...
				if (file.isFile() && file.exists()) { // 判断文件是否存在
					...
					while ((lineTxt = bufferedReader.readLine()) != null) {
						...
						while (infoList.hasNext()) {
							BasicDBObject info = (BasicDBObject) infoList
									.next();
							String id = info.get("_id").toString();
							if (id == null || id == "") {
								continue;
							}
							time = info.get("time_point").toString();
							if (time == null || time == "") {
								continue;
							}
							...
							// 数据库中存在部分字段值没有缺失，导致整条信息不完整，数值为0的舍弃
							if (aqi > 0) {
								aqiList.add(aqi);
							}
							if (pm25 > 0) {
						...
						subTime = time.substring(0, 10);
						Integer aqiMax = null;
						Integer aqiMin = null;
						Double aqiAve = null;
						// 由于各项值添加进list时舍弃0值，故对list处理时需进行判断
						if (!aqiList.isEmpty()) {
						...
						System.out.println("新信息：" + subTime + "--" + line[0]
								+ "--" + line[1]);
						users_day.save(doc);
